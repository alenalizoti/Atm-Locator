...
@Component({
  standalone: true,
  selector: 'app-qr-code-panel',
  templateUrl: './qr-code-panel.html',
  styleUrl: './qr-code-panel.css',
  imports: [
    CommonModule,
    FormsModule,
    MatCardModule,
...
export class QrCodePanel {
  ...
  atm: Atm;
  qrDetails: QrCodeResponse;
  qrWidth = window.innerWidth * 0.85;
  left_time: number = null;
  receivedNumbers: number[] | null = null;

  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private cdRef: ChangeDetectorRef) {}

  ngOnInit() {
    const storedUserInfo = localStorage.getItem('userInfo');
    this.atm = this.data.atm;
    this.qrDetails = this.data.transactionDetails;
    if (storedUserInfo) {
      ...
      const now = new Date();
      const expiresAt = new Date(this.qrDetails.expires_at);
